...
function harness() {
  ...
  let state: any = {activePane:'bottom',panelOpen:false,bottomOpen:false,bottomSplits:{kind:'leaf',id:'bottom',active:'file',tabs:[{id:'file',type:'editor'}]}}
  ...
  const service = {features:['stateSubscription'],isTabEnabled:() => enabled,getSnapshot:() => ({sessionId,state}),
    ...
    openTab:(_:unknown,scope:any) => {calls.push(`open:${scope.sessionId}`);state.bottomSplits.tabs.push({id:KLINE_TAB_ID,type:KLINE_TAB_ID});state.bottomSplits.active=KLINE_TAB_ID;notify()},
    activateTab:(id:string) => {calls.push(`activate:${id}`);state.bottomSplits.active=id;notify()},
  } as unknown as BetterSidebarService
  return {service,storage,calls,notify,get state(){return state},switchSession:(id:string) => {sessionId=id;state={...state,bottomSplits:{...state.bottomSplits,tabs:[{id:'file',type:'editor'}],active:'file'}};notify()},enable:(value:boolean) => {enabled=value;notify()}}
}
test('default tab is added once without expanding panels or replacing the active tab', async () => {
  ...
  assert.equal(h.state.panelOpen,false);assert.equal(h.state.bottomOpen,false);assert.equal(h.state.bottomSplits.active,'file')
...
test('manual close remains closed across refresh and conversation switches', async () => {
  ...
  await Promise.resolve();h.state.bottomSplits.tabs=[{id:'file',type:'editor'}];h.notify();await Promise.resolve()
...
test('existing workbench chart is not reactivated or duplicated', async () => {
  const h=harness();h.state.bottomSplits.tabs=[{id:KLINE_TAB_ID,type:KLINE_TAB_ID}]
