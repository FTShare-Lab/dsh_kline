...
const standaloneContent = readFileSync(new URL('../src/client/KlineContent.tsx', import.meta.url), 'utf8')
const generatedView = readFileSync(new URL('../src/client/generated-view.ts', import.meta.url), 'utf8')
const notice = html.slice(html.indexOf('function workspaceSectionNotice('), html.indexOf('function renderSecurityWorkspace('))
test('security navigation exposes a separate market and dragon-tiger view', () => {
  ...
  assert.match(nav, /data-security-tab="dragon">龙虎榜<\/button>/)
  ...
  assert.match(nav, /class="security-nav-tab active" id="securityTabMarket"/)
  assert.match(html, /let activeSecurityTab = "market"/)
...
test('market and sector pages have separate data responsibilities', () => {
  ...
  assert.doesNotMatch(market, /marketOverview/)
  ...
  assert.match(loader, /callMarketTool\("fetch_market_pulse", \{ refresh, sections \}\)/)
...
test('board visual grammar uses compact cards and related ETF entry points', () => {
  assert.match(html, /function intelligenceRankMarkup\(/)
  assert.match(html, /market-leaderboard intelligence-leaderboard/)
  assert.match(html, /function relatedEtfsForIndustry\(/)
  assert.match(html, /data-related-etf-symbol/)
  assert.match(html, /银行ETF/)
})
test('market workspace exposes directions and a separate dragon-tiger destination', () => {
  ...
  assert.doesNotMatch(market, /rankings\[marketRankingMode\]/)
  assert.match(market, /data-market-open-dragon/)
  assert.match(market, /function renderDragonWorkspace/)
...
test('dragon tiger tab ranks real five-seat net buying and selling without overlapping overview slices', () => {
  ...
  assert.match(dragon, /五席净卖出前\$\{netSell\.count \|\| 0\}/)
  assert.match(dragon, /five_seat_net_value/)
  assert.doesNotMatch(dragon, /rows\.slice\(0, 10\).*rows\.slice\(-10\)/s)
})
