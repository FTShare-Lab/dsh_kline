...
import { chartStorageKey, latestChart, latestChartFromEvents } from './conversation';
...
export function apply(ctx) {
    ctx.effect(() => {
        ...
        root.render(_jsx(ConversationSidebar, { sessions: ctx.sessions }));
        return () => {
            root.unmount();
            host.remove();
...
function ConversationSidebar({ sessions }) {
    ...
    return _jsx(KlineSidebar, { conversationId: list.current, sessions: sessions }, list.current ?? 'no-conversation');
}
...
function KlineSidebar({ conversationId, sessions }) {
    ...
    const conversation = useSyncExternalStore(listener => face?.subscribe(listener) ?? (() => { }), () => face?.getSnapshot() ?? EMPTY_CONVERSATION);
    const eventSource = binding?.eventSource;
    const events = useSyncExternalStore(listener => eventSource?.subscribe(listener) ?? (() => { }), () => eventSource?.getSnapshot() ?? EMPTY_EVENTS);
    ...
    useEffect(() => {
        if (!conversationId || !discovered || (reference && reference.order >= discovered.order))
            return;
        ...
        if (discovered.order >= mountedAt.current)
    ...
    const setStandaloneIdentity = useCallback((symbol, name) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: `dsh-kline-rail ${state.open ? 'is-open' : ''}`, "aria-label": state.open ? '收起 K 线侧栏' : '打开 K 线侧栏', "aria-expanded": state.open, title: state.open ? '收起 K 线侧栏' : '打开 K 线侧栏', onClick: () => setOpen(!state.open), children: _jsx("span", { "aria-hidden": "true", children: "K" }) }), _jsxs("aside", { className: `dsh-kline-panel ${state.open ? '' : 'is-hidden'}`, style: { width: panelWidth }, "aria-label": "K \u7EBF\u5206\u6790\u4FA7\u680F", "aria-hidden": !state.open, children: [!compact && (_jsx("div", { className: "dsh-kline-resize", role: "separator", "aria-label": "\u8C03\u6574 K \u7EBF\u4FA7\u680F\u5BBD\u5EA6", "aria-orientation": "vertical", "aria-valuemin": PANEL_MIN, "aria-valuemax": PANEL_MAX, "aria-valuenow": panelWidth, tabIndex: 0, onKeyDown: (event) => {
                        ...
                        } })), _jsxs("header", { className: "dsh-kline-header", children: [_jsxs("div", { children: [_jsx("strong", { children: session?.name && !isSymbolLikeName(session.name, session.symbol)
                                            ? session.name
                                            : workspaceIdentity?.name && !isSymbolLikeName(workspaceIdentity.name, workspaceIdentity.symbol)
                                                ? workspaceIdentity.name
                                                : 'K 线分析' }), (session?.symbol ?? workspaceIdentity?.symbol) && _jsx("small", { children: session?.symbol ?? workspaceIdentity?.symbol })] }), _jsx("button", { type: "button", "aria-label": "\u5173\u95ED K \u7EBF\u4FA7\u680F", title: "\u5173\u95ED", onClick: () => setOpen(false), children: "x" })] }), update && (_jsxs("div", { className: "dsh-kline-update", role: "status", children: [_jsxs("span", { children: ["\u53D1\u73B0\u65B0\u7248\u672C v", update.version] }), _jsx("a", { href: update.url, target: "_blank", rel: "noreferrer", children: "\u67E5\u770B\u66F4\u65B0" })] })), _jsx("div", { className: "dsh-kline-content", children: session ? (_jsx(NativeKlineApp, { session: session, conversationId: conversationId ?? 'standalone', onIdentity: (symbol, name) => setSession(current => current ? { ...current, symbol, name } : current) }, session.session)) : available ? (_jsx(StandaloneKlineApp, { conversationId: standaloneWorkspaceScope(conversationId), onIdentity: setStandaloneIdentity }, standaloneChartSession(conversationId))) : (_jsxs("div", { className: "dsh-kline-empty", role: "status", children: [_jsx("strong", { children: "\u56FE\u8868\u670D\u52A1\u672A\u8FDE\u63A5" }), _jsx("span", { children: "\u8BF7\u68C0\u67E5 dsh_kline MCP \u72B6\u6001" })] })) })] })] }));
}
