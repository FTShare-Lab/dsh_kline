...
import { PACKAGE_VERSION } from './generated-view';
import { useChartReference } from './chart-session-resolver';
import { isNewerStableVersion } from './version';
import { NativeKlineApp, StandaloneKlineApp, standaloneChartSession, standaloneWorkspaceScope, isSymbolLikeName } from './KlineContent';
const PANEL_MIN = 420;
const PANEL_MAX = 920;
...
export function ClassicShell({ sessions }) {
    useEffect(() => {
        installStyles();
        return () => {
...
function KlineSidebar({ conversationId, sessions }) {
    ...
    const reveal = useCallback(() => setState(current => ({ ...current, open: true })), []);
    const reference = useChartReference(sessions, conversationId, reveal);
...
async function checkForUpdate() {
    try {
        ...
        if (release.prerelease === true || !url.startsWith('https://github.com/FTShare-Lab/dsh_kline/releases/') || !isNewerStableVersion(version, CURRENT_VERSION))
            return undefined;
        return { version, url };
    }
    catch {
