...
const STANDALONE_CHART_PAYLOAD = {
    ...
    // A new conversation starts at the Market page. Keep a dormant default
    // instrument so an explicit click on the empty Chart page has something
    // useful to open; launcher initialization itself must not load it.
...
export function standaloneChartSession(conversationId) {
    return `standalone:${conversationId ?? 'global'}`;
}
export function standaloneWorkspaceScope(conversationId) {
    return `direct:${conversationId ?? 'global'}`;
}
export function isSymbolLikeName(value, symbol) {
...
export function NativeKlineApp({ session, conversationId, onIdentity }) {
    ...
    return (_jsx("div", { className: "dsh-kline-native-host", style: { width: '100%', height: '100%', overflow: 'hidden' }, ref: host, children: error && _jsx("div", { className: "dsh-kline-error", role: "alert", children: error }) }));
}
export function StandaloneKlineApp({ conversationId, onIdentity }) {
    const host = useRef(null);
    const interfaceMode = { ...useContext(InterfaceContext), onboardingKind, acknowledgeOnboarding };
    const [error, setError] = useState('');
    useEffect(() => {
