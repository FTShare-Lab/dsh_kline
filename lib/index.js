...
async function serveRuntimeSession(request, response) {
    ...
    try {
        ...
        if (pathname === '/dsh-kline/session' || pathname === '/dsh-kline/data') {
            ...
            const saved = JSON.parse(await readFile(join(SESSION_DIR, `${token}.json`), 'utf8'));
            if (saved.session !== token || saved.ok !== true)
                throw new Error('invalid_chart_snapshot');
        ...
        sendJson(response, code === 'ENOENT' ? 200 : 500, code === 'ENOENT'
            ? (pathname === '/dsh-kline/data' || pathname === '/dsh-kline/session'
                ...
                : unavailableService())
            : { ok: false, error: 'chart_session_manifest_failed' }, request.method === 'HEAD');
    }
}
...
function chartServiceHeaders(session) {
    return { 'X-DSH-Kline-Token': session.service_token };
}
async function readLiveSession() {
    const payload = JSON.parse(await readFile(RUNTIME_FILE, 'utf8'));
    return isLiveSession(payload) ? payload : undefined;
}
function serviceOrigin(session) {
    return new URL(session.service_url).origin;
}
async function proxyJson(response, url, init) {
...
function isLiveSession(value) {
    ...
    try {
        ...
        if (serviceUrl.protocol !== 'http:' || !['127.0.0.1', 'localhost'].includes(serviceUrl.hostname))
            return false;
    }
    catch {
        return false;
...
function sendBytes(response, status, body, contentType, head = false) {
