...
默认“自动”：检测到可用的 Better Sidebar 时，在工作台的 `+` 菜单中选择 **K线分析 / K-line**；未安装框架时继续使用原来的右侧 K 按钮。图表右上角 **界面**（自选与设置之间）可选择自动、经典独立侧栏或 Better Sidebar Tab，保存并刷新生效。

Tab 模式下请先选择一个对话；图表跟随 Tab 所属对话，而不是全局最后一次分析。两个界面共用原有自选、标注和图表偏好，不改动 FTShare Key。选择 Tab 但框架不可用时会临时回退经典界面并说明原因，不会自动安装插件。已验证的框架版本为 `dsh-better-sidebar 0.18.0`，Harness 保持 `0.1.2-rc.1`。

“分析完成后自动打开 K 线 Tab”可单独关闭。开启时会创建或激活本对话 Tab，但不会强行展开已收起的工作台面板；可通过框架入口手动展开。首次使用保留常规欢迎页，旧用户只显示一次界面升级说明，之后仍可随时通过“界面”切换。

`v0.2.0-rc.1` 是预发布版本，稳定版 `v0.1.8` 保留用于回退；不需要迁移或清空本地数据。
