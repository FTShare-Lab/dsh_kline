...
- **指数行情**：接入 FTShare SDK 1.0.0 的 A 股指数日 K，可用“对比”叠加沪深300、上证指数等指数走势（分钟级指数随套餐提供）。
...
右上角“设置”中的“数据源”可以粘贴 FTShare API Key 并测试连接。未配置 Key 时，A 股日、周、月、季、年 K 及基于这些数据的指标仍可用；港股、美股日线以及各市场分钟 K，则取决于当前 FTShare 套餐是否具备相应权限。Key 只是身份凭据，不会自动授予全部数据能力；新闻、实时行情等能力也以具体接口的套餐标注为准。配置会在当前 dsh 进程生效；勾选保存到本机后，重启会自动加载。Key 不回显、不写入图表状态；也可用 `FTSHARE_API_KEY` 环境变量覆盖。未配置 FTShare 时，仍可使用免费数据和 `analyze_kline_rows` 接入其他数据源。
