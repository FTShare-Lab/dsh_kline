...
在 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 中启用 `dsh_kline` 后，可像与分析助手对话一样提出需求，也可直接点击右侧的 K 按钮，在搜索框中输入股票名称或代码。无需记住固定代码格式。
...
分析完成后，可在图表侧栏继续切换周期、指标、新闻和简况；不经过对话直接搜索的标的也可使用相同工作台。需要区间统计时，依次点击两根 K 线即可。
搜索框可直接输入股票名称或代码；点击星标可加入自选，自选列表支持分组、排序和批量打开。自选数据保存在当前浏览器本机。

右上角“设置”中的“数据源”可以粘贴 FTShare API Key 并测试连接。FTShare 当前免费套餐包含股票 K 线、公司/财务基础资料、A 股行情列表、基金和指数等接口，但免费套餐同样需要注册并配置 API Key；分钟历史行情、港股 K 线、公告/研报和新闻等能力按当前套餐开放，具体以官方文档为准。Key 仅授予账户已开通的能力。配置会在当前 dsh 进程生效；勾选保存到本机后，重启会自动加载。Key 不回显、不写入图表状态；也可用 `FTSHARE_API_KEY` 环境变量覆盖。未配置 FTShare 时，仍可使用内置公开行情兜底和 `analyze_kline_rows` 接入其他数据源。
