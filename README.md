...
插件会先自动使用当前 DSH 服务已经可读取的 `FTSHARE_API_KEY` 或明确指定的凭据文件；设置页会说明来源，但不会扫描 `.env`、shell 配置或钥匙串，也不会显示 Key 或路径。需要覆盖时，可在右上角 **设置 → 数据源** 粘贴 FTShare API Key 并测试连接；可选择仅本次使用，或保存到本机供下次启动加载。
