// The host's session store is the identity authority. Never inspect DOM titles
// or scan another conversation for a chart reference.
export interface ChartReference { session: string; order: number }
export interface Observable<T> {
  getSnapshot(): T
  subscribe(listener: () => void): () => void
}
export interface ConversationSnapshot { nodes?: readonly any[] }
export interface SessionEventWindow { entries: readonly { type: string; event: any }[] }
export interface Sessions {
  list: Observable<{ current?: string }>
  binding(id: string): {
...
export function latestChartFromEvents(entries: SessionEventWindow['entries']): ChartReference | undefined {
  const calls = new Map<string, { name: string; time: number }>()
  const nodes: any[] = []
  ...
  for (const entry of entries) {
    ...
    if (event?.type === 'tool/result') {
      for (const block of data?.message?.content ?? []) {
        ...
        if (!call) continue // Never guess a truncated call's identity.
...
export function latestChart(nodes: readonly any[]): ChartReference | undefined {
  let latest: ChartReference | undefined
  const visit = (node: any) => {
    ...
    if (!latest || order >= latest.order) latest = { session: match[1], order }
  }
  for (const node of nodes) visit(node)
  return latest
}

export function chartStorageKey(conversationId: string): string {
