...
import { KLINE_TAB_ID } from './sidebar-integration.ts'

export const defaultTabKey = (sessionId: string) => `dsh-kline:default-tab:v1:${sessionId}`
...
export function installDefaultKlineTabs(service: BetterSidebarService, storage?: Pick<Storage, 'getItem' | 'setItem'>): () => void {
  if (!service.features?.includes('stateSubscription') || typeof service.getSnapshot !== 'function'
    ...
    || typeof service.isTabEnabled !== 'function') return () => {}
  let stopped = false, pending = false
  const handled = new Set<string>()
  try { storage ??= globalThis.localStorage } catch { /* In-memory fallback for blocked storage. */ }
  const schedule = () => {
  ...
  try { unsubscribe = service.subscribeState(schedule) } catch {}
  schedule()
  return () => { stopped = true; try {unsubscribe?.()} catch {} }
}
