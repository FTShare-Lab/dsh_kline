...
import { NativeKlineApp, StandaloneKlineApp, standaloneChartSession, standaloneWorkspaceScope, isSymbolLikeName, type ChartSession, type ChartIdentity } from './KlineContent'
...
export function ClassicShell({ sessions }: { sessions: Sessions }) {
  useEffect(() => {
    installStyles()
    return () => {
...
async function checkForUpdate(): Promise<ReleaseUpdate | undefined> {
  try {
    ...
    if (release.prerelease === true || !url.startsWith('https://github.com/FTShare-Lab/dsh_kline/releases/') || !isNewerStableVersion(version, CURRENT_VERSION)) return undefined
    return { version, url }
  } catch {
    return undefined
  }
