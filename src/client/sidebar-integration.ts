...
export function cleanClassicTabs(service: BetterSidebarService): () => void {
  ...
  const schedule = () => {
    ...
    queueMicrotask(() => {
      ...
      const visit = (node: typeof state.splits) => {
        if (node.kind === 'split') node.children.forEach(visit)
        else node.tabs.forEach(tab => { if (tab.type === KLINE_TAB_ID) ids.add(tab.id) })
      }
      visit(state.splits)
      visit(state.bottomSplits)
      state.floats?.forEach(frame => { if (frame.tab.type === KLINE_TAB_ID) ids.add(frame.tab.id) })
