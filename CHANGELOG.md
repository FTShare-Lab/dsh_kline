# Changelog

本项目遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)。

## 0.4.0 - 2026-09-21

- One shared runtime and upstream chart frontend, with independent DSH and Codex MCP Apps entry points.
- Portable Codex plugin bundle with no fixed server, username or source-checkout dependency.
- MCP Apps-only icon button and internal UI action bridge; original 14 model-facing tool schemas preserved.
- DSH and Codex now use separate strict package allowlists; neither package carries the other host's UI assets or private validation records.
- Filesystem-only packaging; FTShare v1.0.7 matches the upstream dependency.
- MCP Apps host context drives the shared frontend's theme/language preferences; both UI transports preserve credential provenance.

## [Unreleased]

## 0.3.4 - 2026-09-21

### Added

- 宿主现在可显式以 `DSH_KLINE_RUNTIME_MODE=external` 和 `DSH_KLINE_PYTHON` 提供已配置的 Python 运行时；插件会验证 Python 版本和必需依赖后直接使用它，不会创建虚拟环境、安装依赖或静默回退。

### Fixed

- 修复 Windows 嵌入式 Python 在隔离导入路径下无法定位插件同目录模块的问题，提升桌面宿主中的启动稳定性。

### Compatibility

- 未设置外部运行时模式时，继续使用原有的 `auto` 运行方式；现有安装和 FTShare 配置无需调整。

## 0.3.3 - 2026-09-17

### Improved

- 数据源设置会明确说明 FTShare Key 的自动复用顺序和安全边界；“更改配置”改为设置面板内的独立下拉项，避免与宿主输入法或自动补全浮层重叠。

### Fixed

- FTShare 适配契约与 CI 测试统一校验已固定安装的 SDK `v1.0.7`，修复跨平台安装与发布打包流程中的旧版 `v1.0.3` 断言失败。

## 0.3.2 - 2026-09-17

### Added

- FTShare 设置会安全识别当前 DSH 服务已经可用的凭据来源：宿主环境、明确指定的凭据文件、插件本机保存或仅当前进程；界面会解释当前状态，但绝不回显 Key 或路径。
- 主题和语言均新增“跟随宿主（默认）”。插件会读取宿主的语言与主题偏好，并允许用户分别手动覆盖。

### Fixed

- 仅插件本机保存的 FTShare Key 可以在设置页清除；宿主环境和外部凭据文件保持由用户/宿主管理，避免误删。
- 更新 FTShare SDK 固定版本至仍可下载的 `v1.0.7`，修复新环境安装时旧版归档地址返回 404 的问题。

## 0.3.1 - 2026-09-16

### Improved

- 选中单根 K 线时，浮层卡片改为紧凑的左右对齐明细表，展示时间、开高低收、涨幅、成交量和成交额；价格按涨跌语义着色，关闭操作收敛为右上角叉号。
- 区间统计改为图表内自适应卡片，复用单根 K 线的明细表模板；只保留开始/结束时间、区间涨跌、振幅、最大回撤、成交量、均量和 K 线数。
- 区间统计卡会锚定所选区间，自动选择上方或下方空间并避开图表边界；缩放、滚动和调整图表大小后会重新定位。

### Fixed

- 区间统计完成后自动收起选择提示条，并清除旧的图表底部统计卡与表格，避免同一结果在多个位置重复出现。

## 0.3.0 - 2026-09-14

### Changed

- 本地开发基线升级至 DeepSeek Harness `0.1.5-rc.2` 与 Better Sidebar `0.19.1`；新版工作台下，原生 K 线 Tab 仅在用户明确选择 Tab 界面时自动打开，避免抢占新对话。
- 经典独立侧栏恢复为默认且始终可达；即使新版工作台尚未创建右侧容器，新对话也保留 K 按钮入口。
- 插件名称跟随系统语言：中文为“非凸 K 线助手”，其他语言为 “FtAI K-Line”。

### Improved

- 市场首页的八个主要指数固定保留卡槽，并改为分批填充：最先返回的指数立即显示，其余指数继续加载，不再等待最慢的数据源后整组出现。
- 单个指数短暂失败时只影响对应卡槽；已有有效报价继续保留，避免刷新时整组闪空。

## 0.2.9 - 2026-09-11

### Fixed

- Keep deferred Python runtime bootstrap detached across POSIX and Windows hosts so a short-lived MCP handshake cannot terminate the installer.
- Track the bootstrap owner PID and clean up stale markers after interrupted launches, allowing automatic reconnect to recover normally.
- Make the isolated DSH profile smoke test independent of Corepack's temporary-project mutation behavior.

## 0.2.8 - 2026-09-11

### Added

- 本地证券目录扩展至全量 A 股、港股、美股与 ETF；搜索支持名称、代码、别名及常见交易所后缀，港股 `700`、`0700` 与 `00700.HK` 等价。

### Fixed

- 日 K 历史分页现在遵守 FTShare 单次最多 1,000 行的限制；“今年”等较长周期不再因请求参数超限返回 HTTP 400。

## 0.2.7 - 2026-09-11

### Changed

- 普通 A 股的当日分时改为优先使用 FTShare v4 实时分钟 K 线，直接绘制原始一分钟数据，不再做二次聚合。

### Fixed

- v4 返回的按标的 `items` 数据包现在会正确解析；接口暂不可用或尚未形成完整交易日时安全回退到 v2 历史分钟线。
- 指数、ETF 与多日分钟历史继续使用各自已验证的 v2 接口，避免 v4 覆盖范围较窄时造成图表缺失或混用不同周期数据。
- DSH Web 首次安装时，Python 运行环境会在后台准备；DSH 不再因 MCP SDK 的初始 60 秒握手超时而退出，依赖完成后会自动重连并加载工具。

## 0.2.6 - 2026-09-10

### Changed

- 自选标的总上限从 48 提高到 200，前端本地数据和 MCP 持久化服务使用同一上限。
- 市场首页固定展示八个主要指数；数据尚在加载时保留稳定卡槽，避免不同数据返回量造成页面跳动。
- 市场首页优先加载指数，延后非关键预取，缩短首次打开时看到行情的等待。
- 涨跌颜色改为全局主题设置：默认红涨绿跌，可在设置中切换为国际市场的绿涨红跌，并在刷新和新对话后保持。
- 自选侧栏的分组操作收敛为紧凑图标按钮；批量导入改为带实时预览的对话框。

### Added

- 自选分组支持重命名。
- 市场、板块、资讯、公司和龙虎榜页面共用响应式信息卡布局；龙虎榜在可用宽度内使用两列展示。

### Fixed

- FTShare 冷启动导入和板块接口限流会有限重试；短暂异常不再把已加载的板块或龙虎榜清空。
- 市场指数按单个标的回退并保留最近一次有效报价，避免部分数据源短暂缺项时整条指数行情被替换或遗失。
- 修复经典独立侧栏收起后自选栏残留文字覆盖工作台标题的问题。
- 公司资料遇到限流时不再重复展示多个错误卡片：已加载内容保留，后台自动重试一次，并在服务端停止同批重复请求。

## 0.2.5 - 2026-09-09

### Fixed

- 修复 v0.2.4 新会话首先显示市场后，用户直接点击空的“图表”页不会展示默认行情的问题；现在仅在该操作下懒加载上证指数，搜索、自选和市场卡片的明确标的跳转不受影响。

## 0.2.4 - 2026-09-09

### Added

- 新增独立“龙虎榜”一级页面，按真实五席净额分别展示净买入与净卖出排行；明细缺少名称时使用同源总览、标的目录或精确搜索补齐，不重复计算同时出现在买卖列表中的营业部。

### Changed

- 新会话默认进入市场首页且不预选上证指数；只有当前会话新产生分析结果或用户主动选择标的时才进入图表，避免 Better Sidebar 复用旧 scope、缓存或历史消息造成跨会话串图。
- 市场情报改为渐进加载：指数先展示，市场宽度、资金与板块优先返回，排行及龙虎榜随后独立加载；可选接口失败或限流不再阻塞整个市场首页。
- 行业、概念、资金历史与股票排行统一使用紧凑信息卡，并改善龙虎榜窄侧栏布局、加载占位和点击进入图表的行为。

### Fixed

- 修复新增 MCP 工具后发布冒烟测试仍写死工具总数，导致功能正常但 Windows、Ubuntu 与 Release CI 被误判失败的问题；现在校验必须存在的工具集合，并允许向后兼容地增加工具。

## 0.2.3 - 2026-09-09

### Added

- 新增“市场”首页：集中展示内地、香港与美国的主要指数、市场宽度、行业／概念热度、股票人气与飙升排行，以及龙虎榜；可直接打开相应标的的 K 线。
- 新增“板块”研究页：集中呈现市场脉搏、行业与同类股、主力资金强弱、可关联的 ETF，以及可用的交易事件；相关标的可直接打开行情。
- 热门行业与概念可打开独立板块详情，展示已验证的资金快照与资金历史；无法可靠匹配的成分股数据会明确标记为取决于数据源。
- 新增 ETF 标的识别与行情支持，ETF 可通过搜索、自选和图表工作台使用。
- 自选升级为用户级常态化数据：不同对话和工作台入口读取同一份自选与分组，关闭后再次打开仍可恢复。
- 关键点位分析后新增支撑、压力、突破、风险与目标五类参考卡；每张卡可一键保存为自己的图表点位。

### Changed

- 工作台一级页面收敛为“市场、图表、板块、资讯、公司”。公司画像、财务表现和股东与筹码在“公司”页连续呈现；旧版入口会自动迁移，避免升级后出现空白页。
- “新闻”更名为“资讯”，不再在页面名称中显示数量；板块页以指标卡、涨跌颜色和资金强弱条呈现数据，便于快速阅读。
- 新增数据源能力契约：不同数据源分别声明行情、市场、板块、公司与研究数据的真实可用范围，外部 OHLCV 不会被误当作完整市场数据源。
- 自选同步采用版本与更新时间判断；离线修改会保留在本机并在下一次可用时自动同步。发生并发修改时会合并双方新增内容，避免一个对话覆盖另一个对话的新增自选。
- 自选侧栏显示最新价与日涨跌幅，支持按涨跌幅排序，并显示当前分组的上涨、下跌与平均涨跌概览。

### Fixed

- 修复板块数据误取历史首条、区域板块混入行业板块，以及把资金历史错误渲染为重复行业／股票名称的问题。
- 修复发布包可能夹带 Python 缓存文件的问题，发布产物保持干净且可复现。

## 0.2.2 - 2026-09-09

### Added

- 新增原生 Windows 普通用户安装支持：使用 DSH 自带的 Node 启动 MCP，识别 Windows Python Launcher，并适配 `%LOCALAPPDATA%`、`Scripts/python.exe`、空格及中文路径，无需 Git Bash 或 WSL。
- 新增 Ubuntu/Windows 真实安装 CI；发布包会在隔离 DSH Web profile 中完成首次启动、MCP 和图表服务冒烟验证。

### Fixed

- Windows Python 3.10–3.12 保存 FTShare Key 时不再调用尚未支持的 `os.fchmod`，也不再错误套用 POSIX mode-bit 校验。
- 对 Windows 短暂文件占用增加有上限的重试，降低杀毒软件扫描或文件锁造成的凭据、图表会话写入失败。
- Windows 首次安装在用户名或插件路径含中文时，Python 与 pip 统一使用 UTF-8，不再因系统默认代码页无法输出路径而中断。

### Changed

- 受管 Python 环境、安装日志与图表运行文件统一放入平台用户目录；可分别使用 `DSH_KLINE_CACHE_DIR` 和 `DSH_KLINE_RUNTIME_DIR` 覆盖。
- `pnpm bootstrap` 改用跨平台 Node 入口；原有 shell 文件保留为 macOS/Linux 兼容包装。

## 0.2.1 - 2026-09-09

### Fixed

- 普通用户首次启动时自动准备用户级 Python 运行环境；依赖变更、安装中断或环境不完整时可自动续装和修复，并保留可定位的安装日志。
- 发布包采用明确的运行文件白名单，不再夹带项目 `.venv`、`node_modules`、测试文件或本地缓存。

### Changed

- GitHub Release 增加固定名称的 `dsh-kline.tgz` 和 SHA-256 校验文件；发布流程会校验 tag 与插件版本、执行构建和全量 Python 测试后再上传资产。

## 0.2.0 - 2026-09-08

### Added

- 新增双界面模式：自动适配可用的 Better Sidebar 工作台 Tab；未安装框架时继续使用经典独立侧栏。两种界面共用图表内容、用户标注、自选与数据源配置。
- 新增界面选择与首次使用引导，支持自动、经典与 Tab 三种偏好；用户可随时切换，框架不可用时会安全回退。

### Fixed

- 修复 FTShare 美股日线无日期参数时仅返回最新一根导致 NVDA 等标的加载失败；补充历史查询上界，并按纽约交易日期归一化。美股分钟接口不支持时明确说明市场限制，不再误导用户重复配置 Key。
- 修复 FTShare 港股历史行情的日期格式与周期聚合：腾讯控股等标的可正常取得日、周、月、季、年线；长周期基于最新日线本地聚合，避免供应商原生长周期的陈旧数据。
- 修复同一安装目录启动多个 DSH/MCP 实例时，共享服务定位文件被临时实例覆盖、退出后造成 `chart_session_unavailable` 空图的问题。服务定位按宿主进程隔离，图表仍按原有对话 token 寻址。
- 修复 tarball 安装丢失脚本执行位后首次启动失败的问题：显式通过 Bash 调用 bootstrap，并增加全新环境、0644 权限、中文/空格路径的真实安装冒烟脚本。
- 兼容旧版或不完整的 Better Sidebar 布局，隔离清理异常，明确 Tab 注册的卸载清理函数。

### Changed

- Tab 模式首次进入每个对话时添加一次 K 线入口，不展开面板、不抢占当前标签；用户关闭后记住选择，仍可通过 `+` 菜单重新打开。分析后的自动打开偏好独立保留。
- 界面入口改为无图标的纯文字 `UI`，窄栏中仍显示；品牌统一为“非凸K线助手 / dsh_kline”，自动迁移当前对话里保存的旧默认 Tab 标题，保留自定义名称。

## 0.2.0-rc.1 - 2026-09-08

### Added

- 新增双界面模式：默认自动检测可用的 Better Sidebar，使用原生“K线分析 / K-line”Tab；未安装框架时继续使用经典独立侧栏。
- 图表右上角新增“界面”入口，可保存自动、经典、Tab 三种偏好并刷新切换；框架不可用时解释回退原因，不自动安装框架。
- 分别提供首次使用欢迎与一次性升级说明，支持中英文、键盘选择、Escape 关闭和焦点返回。
- Tab 按所属对话隔离图表，隐藏时停止图表运行并保存视图，重新显示时恢复；自动打开 Tab 可单独关闭，遵循框架公共 API，不强制展开收起的面板。

### Changed

- 经典与 Tab 外壳共用图表内容、对话解析和原有存储键。仅经典模式安装独立侧栏的布局样式，避免双侧栏竞争和模式切换后的样式残留。
- 保持 Harness `0.1.2-rc.1` 与现有 MCP、数据源配置、鉴权模型不变。Better Sidebar `0.18.0` 仅作为开发类型依赖，不是运行必装项。
- 本版本为预发布验证版；稳定用户仍可继续使用 `v0.1.8`。

## 0.1.8 - 2026-09-08

### Changed

- 分时 5D 改为按完整交易日预取并缓存；打开标的后会在后台加载可用的五日分钟历史，后续切换无需重复等待。

### Fixed

- 修复 FTShare 分钟历史分页从开盘边界向前翻页时只返回一天数据的问题。5D 现在从前一交易日收盘分页，并只将完整交易日计入历史，避免边界 bar 造成假 5D 或错误降级为 1D。
- 临时配置 FTShare API Key（`persist=false`）不再删除用户已保存的本机 Key；只有显式清除持久配置才会移除凭据文件。
- 修复插件安装目录含空格或非 ASCII 字符时，Cordis 启动路径被 URL 编码而无法启动的问题。
- 移除意外同步的 `“ 2”` 重复副本，并阻止此类本机恢复文件进入后续发布包。

## 0.1.7 - 2026-09-07

### Fixed

- 修复从 GitHub 或插件市场标准安装时只挂载侧栏、没有启动行情 MCP 服务的问题：服务配置现随插件 bundle 一起安装，并从 profile 中已安装的包目录定位启动脚本。
- 首次标准安装不再要求用户手动创建项目内 `.venv`：启动时会在用户缓存目录准备独立 Python 运行环境；已有开发目录的 `.venv` 和显式 `DSH_KLINE_PYTHON` / `DSH_KLINE_VENV` 配置仍优先使用。
- 启动器会跳过 macOS 可能自带的 Python 3.9，自动选择可用的 Python 3.10+；没有合格解释器时给出可执行的配置提示。
- 开发启动脚本不再额外挂载旧 MCP 补丁，避免与 bundle 内服务配置重复注册。

## 0.1.6 - 2026-09-07

### Added

- 支持从右侧 K 按钮直接进入独立工作台：新对话默认展示上证指数，用户可搜索标的、查看 K 线并管理自选，不必先发起对话分析。
- 图表会话按对话隔离并使用不可变快照保存；不同对话的分析结果、侧栏图表和本机状态不会互相覆盖。
- 新增“清除标注”分类菜单，可分别清除分析点位、手动画线或自定义点位；全部清除需要确认，且只影响当前对话和当前标的。
- 公司简况补充公司简介、上市日期、主营业务、主要产品和所属板块；资料区域可区分空结果、部分可用、权限、限流和未接入状态。

### Changed

- 升级 DeepSeek Harness 兼容层至 `0.1.2-rc.1`，适配新的会话事件与客户端注入接口。
- 图表画布按宿主可用高度自适应；手动调整高度后可在设置中恢复自适应，也可双击拖动条恢复。
- 原有顶部刷新按钮现在同时刷新行情和公司资料，并绕过过期预取缓存；不新增单独的“重新加载资料”入口。
- FTShare 设置与文档明确区分匿名兜底和配置免费 API Key 后的套餐能力；免费套餐接口仍需注册并配置 Key。

### Fixed

- 修复年线历史不足、周/月/季跨页重复 K 线及由此引起的成交量和技术指标失真。
- 修复完全平盘序列被错误判定为 RSI=100 和超买的问题。
- 修复日 K 已可用时，分钟线权限不足仍让 FTShare 连接测试整体失败的误导提示。
- 修复常见 A 股名称/代码首次搜索为空、裸代码展示不一致及回看深度未提示的问题。
- 关键点位现在基于当前已加载 K 线本地计算，不重复请求行情源，也不会重置周期、复权、指标或缩放；晚到结果不会覆盖已切换的标的或已清除的标注。
- 修复公司资料查询向 `company_list` 传递交易所后缀导致空结果的问题；指数不再被误当作上市公司查询。
- 修复侧栏高度在大窗口留下空白、已清除标注刷新后复活及清除菜单在小窗口被裁切的问题。

## 0.1.5 - 2026-09-07

### Fixed

- Pass the loaded KLineCharts dependency explicitly into the scoped chart runtime instead of relying on a cross-realm global lookup.
- Ship the browser runtime in the repository so direct GitHub installs and updates do not depend on blocked lifecycle build scripts.
- Make failed vendor loads retryable and replace the obsolete inline vendor placeholder with a working standalone preview script.

## [0.1.4] - 2026-09-05

### Added

- 底部大盘行情条接入官方全球指数端点：恒生/纳指/上证/沪深300/深成 5 条指数在配置 FTShare Key 后全部走官方数据（A 股指数走 `index_candlesticks`，全球指数走新增登记的 `global_index_daily_kline`）；大盘条条目可点击，直接打开对应指数的 K 线图。
- 内置免费大盘兜底（`tools/free_sources.py`）：无 Key/匿名或官方接口不可用时，大盘条自动回退腾讯公开行情快照，指数日 K 回退东方财富（再兜底腾讯），保证免费用户仍能看到大盘数据；所有免费回退数据均标注 `eastmoney_free` / `tencent_free` 来源与链接，不冒充 FTShare。

### Changed

- 将 FTShare Python SDK 从 1.0.0 升级到 1.0.3，并将默认 `/gateway` 基址、免费/基础/进阶/专业套餐边界同步到官方文档。
- 免费版股票日 K、指数日 K 和证券目录能力重新执行匿名实测；需付费权限的股票/指数历史分钟、港股与新闻能力仅按官方 SDK 契约对齐，不标记为实盘验证通过。
- `fetch_candles` 识别全球指数（`100.*`）并路由到 FTShare 官方日 K 端点（周/月/季/年本地聚合）；`data_source_status` 与设置页数据源信息新增内置免费源能力摘要。

### Fixed

- 将指数历史分钟行情路由从旧的 `api/v3` 对齐为 1.0.3 的 `api/v2`，调用时转换为接口要求的 `.SH/.SZ/.BJ` 短后缀并移除不支持的复权参数。
- SDK 分钟接口显式请求 Python 行数据，避免默认 DataFrame 返回类型绕过适配层约定。
- 兼容 FTShare 1.0.3 免费指数目录的双层 `data.records` 响应包装，避免目录刷新静默丢失指数条目。
- 修复 `scripts/bootstrap.sh` 在受支持 Python 版本上仍会抛出 `TypeError` 的条件退出写法。

## [0.1.3] - 2026-09-04

### Added

- FTShare SDK 1.0.0 适配：实测并登记 A 股指数日 K 端点 `index_candlesticks`（api/v1/market/data/index-candlesticks，免费可用），沪深300/上证/深成指等指数对比预设随之启用；指数分钟（`index_minutes`）依赖套餐，无 Key 时返回可执行提示。
- 标的名称旁新增收藏星标：点击即可将当前标的加入/移出当前自选分组，状态随浏览器本机保存，并与图表右键菜单、自选栏保持一致。
- 新增“点位”标注能力：可在图表上添加任意价格的水平价位线，或在任意 K 线上放置文本标注，支持名称/备注/颜色，行内改价、改名、单条删除与一键清空。
- 关键点位标注可编辑：点击“关键点位”后，自动识别的支撑/压力进入可编辑列表，可改价（名称自动同步）、改名、删除或“转我的”固化为自定义点位，不会被后续重分析冲掉。
- 补齐侧栏“关键点位”按钮的后端通道：新增 `analyze_kline` 的 loopback chart API action 并在 web 白名单放行（此前点击必然返回 `unsupported_chart_action`）。
- 对比面板指数预设按数据源能力控制：`data_source_status` 新增 `index_kline` 能力字段；数据源不支持 A 股指数 K 线时，“沪深300/上证”预设自动禁用并提示原因。
- 点位数据随浏览器本地持久化（工作区存储升级为 v8，兼容旧版本读取），并纳入“导出/导入本机数据”。
- provider-neutral 测试补充 chart API `analyze_kline`（支撑/压力标注命令、参数校验、取数错误透传）与 `index_kline` 能力位覆盖。

### Fixed

- 修复“清标注”按钮无反应的问题：现在会清除自动支撑/压力等分析标注与选区（含点位面板中的自动列表），保留自定义点位与用户画线，并在没有可清除内容时给出提示。
- 修复 MA 与 BOLL 无法同屏显示的问题：蜡烛窗内的指标改为叠加创建，二者（及 VWAP）可同时显示。
- 修复侧栏“关键点位”按钮始终失败的问题（补齐 chart service `analyze_kline` 与 web 层白名单）。
- 修复对比预设指数在当前数据源必然失败却可点的问题（改为按能力禁用而非点击后报错）。
- 修复自定义价位/文本标注价格输入在列表中被千分位格式化污染的问题（数值框使用原生数字值）。
- 补充 DeepSeek Harness 插件市场接入说明和界面截图；用户可在插件市场搜索并安装 `dsh_kline`，发布新版本后可从同一处同步并更新插件。
- 增加受控的 FTShare 契约注册表与通道记录：仅在已核实的官方路径和 SDK 候选之间有限回退；认证/套餐权限不换通道，限流仅在原通道退避重试，并补充 SDK 漂移错误分类与回归测试。
- 在数据源状态中公开候选通道的验证标记；未取得分钟权限前不把分钟 GET/SDK 候选标记为真实通过。
- 明确契约候选验证标记随每次发版或 SDK 升级重新核验；日 K 的 SDK 候选已纳入已验证通道。
- 连接测试改为先验证基础行情，再单独探测分钟行情权限；免费版 Key 不会因分钟线未开通而被误报为无效，并返回安全的能力状态。
- 新增本地证券目录驱动的标的名称解析，支持中文名称、裸代码和常见交易所代码格式；新增 `search_symbols` MCP 工具。
- 新增关键点位分析、区间统计、右键加入/移出自选反馈，以及外部数据源提供 OHLCV 后的完整分析工作流。
- 新增英文模式下区间统计、简况、财务和股东信息的字段本地化。
- 新增 `analyze_kline_rows`，支持任意调用方提供的标准化 OHLCV 数据完成指标、图表和侧栏工作流。
- 前端数据源状态栏提示外部数据源接入方式。
- 分钟线在 FTShare 返回 401 时显示配置 `FTSHARE_API_KEY` 的简明提示。
- 保留底部大盘行情区域；暂无大盘数据时显示占位，接入数据源后自动展示。
- 统一数据源提示为“温馨提示”，并在新闻/简况无数据时显示可理解的说明。
- 右上角新增使用帮助，说明分钟线、外部数据源、FTShare 配置、当前版本和更新入口。
- 修复无 API key 时只能搜索内置少量标的的问题，优先使用本地证券目录搜索。
- 明确搜索入口只使用 dsh_kline 本地证券目录，彻底移除 FTShare 搜索接口调用。
- 在帮助面板说明自选与分组的浏览器本地保存边界。
- 设置页增加 FTShare API Key 的配置、清除和连接测试入口；Key 可安全保存到本机并在重启后自动加载，不回显、不写入图表状态。
- 新增 `data_source_status`、`configure_ftshare` 和 `test_ftshare_connection`，为未来支持更多 API 数据源预留统一配置边界。
- 将非法 K 线周期改为中文可执行提示并列出允许值，避免暴露 Pydantic 内部错误和文档链接。
- 修复设置页 FTShare 配置请求未接入 loopback chart service、导致配置操作被误报为不支持 action 的问题。
- 修复美股周/月/季/年线先截断再聚合导致首根 K 线不完整的问题。
- 修复外部 `analyze_kline_rows` 将自定义标的标签错误限制为本地证券目录的问题。
- 统一无数据、数据权限和分钟线不可用时的中文可执行提示，并清理跨标的残留选择状态。
- 移除无引用的本地证券 mock 数据，避免模拟内容被误认为真实行情或资讯。

### Changed

- 默认展示收敛为 MA + VOL + MACD（BOLL/KDJ/RSI/ATR/VWAP 仅在需要时手动开启），默认时间范围为今年（YTD）；仅当分析请求明确指定指标时才自动切换指标栈，旧版偏好在首次打开时迁移一次。
- FTShare 降级为可选适配器；`health` 不再因 FTShare 缺失而判定 dsh_kline 整体不可用。
- 使用 FTShare v1 SDK 的 `stock_minutes` 接口获取分钟线，并保留旧 SDK 兼容回退。
- 设置页集中语言、主题、数据源和帮助，FTShare 配置失败时保留其他数据源工作流。

### Security

- 外部 K 线输入增加有限数值校验和 12000 行输入上限。
- FTShare API key 仅由服务端注入 FTShare 请求，可通过本机凭据文件或环境变量提供，不写入仓库或图表数据。
- 图表 loopback 服务增加进程级 token、Host 白名单和 JSON 请求校验；公开会话与浏览器不持有服务 token。

## [0.1.2] - 2026-08-21

### Added

- 搜索栏支持直接搜索股票代码或名称，并可从搜索结果打开新的标的工作区。
- 支持同时打开多个标的，并在搜索栏下方以独立 tab 栏管理工作区。
- 自选列表支持行情展示、分组、排序和批量打开。

### Fixed

- 标的 tab 从搜索/操作栏移出，避免与品牌、搜索框和操作按钮争抢横向空间。
- 单个标的时自动隐藏 tab 栏，多个标的时保持 tab 宽度稳定并支持横向滚动。
- 稳定图表头部、行情信息和指标值布局，避免与 K 线区域重叠。

### Removed

- 移除仅用于开发验证的 smoke/live 脚本、测试代码和调试命令，精简正式源码仓库。
- 移除 README 展示截图和内部架构文档；前端来源说明保留为根目录的 `PROVENANCE.md`。

## [0.1.1] - 2026-08-20

### Added

- 原生 DeepSeek Harness 侧栏中的版本更新提示与 GitHub Release 链接。
- 中文 README、交互式图表示例和用户使用说明。

### Fixed

- 图表静态资源不再依赖已有图表会话，避免空会话时加载失败。
- 恢复原生 MA 图例的单行布局与完整标签。
- 指标切换后保持当前图表会话数据，避免错误提示。

## [0.1.0] - 2026-08-16

### Added

- 首个独立发布：支持港股、美股和 A 股的 K 线分析 MCP。
- 交互式侧栏、技术指标、支撑压力位、区间统计、新闻与基本面信息。
